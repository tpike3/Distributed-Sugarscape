...
class NetAgent(Agent):
    ...
    def resolve_move(self):
       ...
       # print ([self.assess_sustenance(pos) for pos in neighbors])
        ...
        for pos in neighbors[:]: 
            if pos[0] < 0 or pos[0] >= self.model.grid.width: 
        ...
        if final_candidates[0] == self.pos: 
            ...
            boundary = self.model.grid.move_agent(self,final_candidates[0])
            #self.history.append((final_candidates[0], self.debug, self.model.step_num, \
            #                     self.model.tracker, boundary))
            if boundary != None: 
                raise ValueError ("Boundary Movement issues")
    ...
    def die(self): 
        if self.accumulations[1] <= 0.1 or self.accumulations[2] <= 0.1: 
            ...
            self.model.grid.agent_died(self, self.model.schedule)
    ...
    def step(self):
        ...
        #print (self.model.tracker, self.unique_id)
    ...
    def iter_stuff(self):
        ...
        for row in range(len(self.model.grid.grid)): 
            for cell in range(len(self.model.grid.grid[row])): 
                for i in self.model.grid.grid[row][cell]: 
                    if i.unique_id == self.unique_id: 
                        print ('agent in', row, cell )
